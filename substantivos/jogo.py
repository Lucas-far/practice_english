...
while True:
    ...
    try:
        ...
        box = [mandatory_adjectives, chosen_verb_infinitive, chosen_verb_past, chosen_verb_present, chosen_verb_future]
        box = set(box)
        box_elements = sorted(list(create_set(box, 10, all_inked_elements, True)), key=len)

        empty_set = set({})

        five_words = list(create_set(empty_set, 5, nouns, True))  # function create_set()
