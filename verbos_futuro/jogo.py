...
while True:
    ...
    try:
        ...
        box = [mandatory_adjectives, mandatory_nouns, chosen_verb_infinitive, chosen_verb_past, chosen_verb_present]
        box = set(box)
        box_elements = sorted(list(create_set(box, 10, all_inked_elements, True)), key=len)

        empty_set = set({})

        five_words = list(create_set(empty_set, 5, future, True))  # function create_set()
        five_indexes = [future.index(word) for word in five_words]
        five_translations = [future_pt_br[index] for index in five_indexes]
        ...
        print(hello := greetings('treino de verbos no futuro', index1=3))
