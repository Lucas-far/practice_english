...
while True:
    ...
    try:
        ...
        box = [mandatory_adjectives, mandatory_nouns, chosen_verb_present, chosen_verb_future]
        box = set(box)
        box_elements = sorted(list(create_set(box, 10, all_inked_elements, True)), key=len)

        empty_set = set({})

        five_words = list(create_set(empty_set, 5, past_irregular, True))  # function create_set()
        five_indexes = [past_irregular.index(word) for word in five_words]
        five_translations = [past_irregular_pt_br[index] for index in five_indexes]
        ...
        print(hello := greetings('treino de verbos no passado (irregulares)', index1=3))
